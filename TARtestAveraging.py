...
def compute_median_template(templates):
    if len(templates)<=1:
        return templates
    num_templates = min(len(templates),5)
    len_templates = len(templates[0])
    median_template = [0 for i in range(len_templates)]
    for i in range(len_templates):
        number_ones = 0
        for j in range(num_templates):
            number_ones = number_ones+templates[j][i]
        if number_ones > num_templates/2:
            median_template[i] = 1
...
if __name__ == '__main__':
    ...
    gen_time=[]
    rep_time=[]
    for x in range(min(len(templates),190)):
        templateNum = x
        ...
        if do_cryptography == 0:
            ...
            gen_time.append(gen_end-gen_start)
            person_tpr = []
            median_template = compute_median_template(templates[templateNum][1:])
            hdist = np.count_nonzero([median_template[i] != templates[templateNum][0][i] for i in
                                      range(len(templates[templateNum][0]))])
            hamming_distance.append(hdist)
            rep_start = time.time()
            temp_matches = fuzzext.rep(median_template, encrypted_lockers, seeds, num_processes=1)
            rep_end = time.time()
            rep_time.append(rep_end-rep_start)
